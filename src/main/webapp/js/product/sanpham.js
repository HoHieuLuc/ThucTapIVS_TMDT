    ...
    try {
        ...
        if (danhGiaSPs.length > 0 || danhGiaSPCuaBan.status === 1) {
            const allDanhGiaSPs = danhGiaSPs.map((danhGiaSP) => {
                ...
                const lanSuaCuoi = ngay_sua ? `<span class="text-muted">(Lần sửa cuối: ${ngay_sua.date.day}/${ngay_sua.date.month}/${ngay_sua.date.year} -- ${ngay_sua.time.hour}:${ngay_sua.time.minute})</span>` : ``;
                return `
                    <div class="comment mt-4 text-justify float-left"> <img src="https://i.imgur.com/yTFUilP.jpg"
                            alt="avatar" class="rounded-circle" width="40" height="40">
                        <h4>${ten}</h4> <span>
                        ${ngay_tao.date.day}/${ngay_tao.date.month}/${ngay_tao.date.year}  -- ${ngay_tao.time.hour}:${ngay_tao.time.minute}
                        </span>${lanSuaCuoi}
                        <br>
                        ${so_sao_html}
                        <p>${noi_dung}</p>
